...
export default function AdminCarReservationUpdate(props) {
    ...
    useEffect(() => {
        axios.get(`http://localhost:8080/carreservation/${id}`).then((res) => {
            setData(res.data);
    ...
    function updateCarReservation(event) {
        ...
        axios.put(`http://localhost:8080/carreservation/update/${id}`, data).then(() => {
    ...
    return (
        <div className="container">
            <center>
                <h1 className="text-light mt-5">Car Reservation Update </h1>
                    <form onSubmit={updateCarReservation} className="col-lg-4">
